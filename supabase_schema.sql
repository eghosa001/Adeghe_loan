...
--
-- After running it:
--   1. Create your owner account: Authentication → Users → Add user
--      (email + password). The app signs in with those same credentials.
--   2. Copy the Project URL and anon key into
--      lib/core/cloud/supabase_config.dart.
--
-- RLS note: every policy below allows any *authenticated* user of this project
-- to read/write all rows. That is fine for a single-owner project; do not
