...
bool isSaneCloudRow(String table,Map<String,Object?> row,String pk){final id=row[pk];if(id==null)return false;if(id is String&&(id.isEmpty||id.length>64))return false;if(!isValidSyncTimestamp(row['updated_at'] as String?))return false;for(final column in cloudNumericColumns){final value=row[column];if(value==null)continue;if(value is! num||!value.isFinite||value<0)return false;}for(final column in cloudIntColumns){final value=row[column];if(value==null)continue;if(value is! int)return false;}if(table=='loans'){for(final column in const ['duration_days','duration_weeks']){final value=row[column];if(value==null)continue;if(value is! int||value<1||value>AppConstants.maxLoanDuration)return false;}}final enums=cloudEnumValues[table];if(enums!=null)for(final entry in enums.entries){final value=row[entry.key];if(value==null)continue;if(value is! String||!entry.value.contains(value))return false;}final requiredText=cloudRequiredTextColumns[table];if(requiredText!=null)for(final column in requiredText){final value=row[column];if(value==null)return false;if(value is! String||value.trim().isEmpty)return false;}return true;}
