...
/// Daily Loan Report: one row per daily loan in the selected period with a
/// named-column table, summary strip and PDF/Excel/Print export.
class DailyLoanReportScreen extends ConsumerStatefulWidget {
  const DailyLoanReportScreen({super.key});

  @override
  ConsumerState<DailyLoanReportScreen> createState() =>
      _DailyLoanReportScreenState();
}

class _DailyLoanReportScreenState extends ConsumerState<DailyLoanReportScreen> {
  ...
  Widget build(BuildContext context) {
    final range = ReportDateRange(
        start: _period.start, end: _period.end, loanType: 'daily');
            ...
            return Column(
              ...
              children: [
                ReportMetricStrip(
                  cards: [
                    ...
                    ReportMetricCard(
                        label: 'Efficiency',
                        value:
                            '${d.collectionEfficiency.toStringAsFixed(1)}%',
                ...
                ReportDataTable(
                  ...
                  rightAlignColumns: const [5, 6, 7, 8, 9],
  ...
  Future<void> _export(BuildContext context, String action) async {
    ...
    final cards = <ReportCard>[
      ...
      ReportCard('Savings', CurrencyUtils.format(d.savingsFromOverpayments, symbol: symbol)),
      ReportCard('Customers', '${d.customerCount}'),
    ];

    final data = ReportExportData(
      reportName: 'Daily Loan Report',
