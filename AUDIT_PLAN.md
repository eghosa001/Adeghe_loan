...
| ~~M14~~ | ~~DB double-open + backup closes DB under active providers~~ | ~~`database_service.dart:17-21`, `backup_service.dart:24`~~ | ~~Memoize init future; pause/queue DB access during backup.~~ **Fixed 2026-08-05:** `database` memoizes the in-flight open (single shared connection, failed open retries); new `withExclusiveAccess` gates concurrent callers during a backup's close→swap→reopen cycle; `createBackup`/`restoreBackup` run inside it. Test: `test/database/database_service_exclusive_test.dart`. |
