...
-- Assign a new authoritative version to every existing legacy row that has 0.
...
begin
  ...
  -- A stale offline device carries an older version. Returning OLD makes the
  -- upsert a no-op instead of allowing an old snapshot to overwrite newer data.
  if new.sync_version < old.sync_version then
    return old;
  end if;
  -- Equal version means the client edited the row based on the current server
  -- snapshot. The server assigns the next global version atomically.
  new.sync_version := nextval('public.cloud_sync_version_seq');
  return new;
end;
$$;
...
-- Deletes are conditional on the version the local device actually observed.
-- This closes the stale-offline-delete hole in the old direct DELETE path.
